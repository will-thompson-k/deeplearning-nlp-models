...
class SkipGramNSModel(nn.Module):
    ...
    def __init__(self, vocab_size: int, embedding_size: int, negative_sample_size: int, word_frequency: torch.Tensor):
        ...
        self._input_embedding = nn.Embedding(self._vocab_size, self._embedding_size, padding_idx=0)
        self._output_embedding = nn.Embedding(self._vocab_size, self._embedding_size, padding_idx=0)

        self._input_embedding.weight.data.uniform_(-0.5 / self._embedding_size, 0.5 / self._embedding_size)
        self._output_embedding.weight.data.uniform_(-0.5 / self._embedding_size, 0.5 / self._embedding_size)
